"""_interrupt_current_turn: forced (PTT) interrupt must reach the framework.

Bug: half_duplex PTT tap-to-stop stopped the agent's audio (mixer cancel) but
gave no reply. Root cause: ``_duck_cancel_and_interrupt`` cancels the output
FIRST (is_cancelled=True), then calls ``_interrupt_current_turn(force=True)`` —
which short-circuited on ``is_cancelled`` and never called
``session.interrupt(force=True)``. So the (uninterruptible) speech handle was
never ended, agent_state stayed "speaking", and the captured barge-in turn
couldn't commit. force must bypass that short-circuit.
"""
...
def test_forced_interrupt_fires_even_when_output_cancelled() -> None:
    # Explicit PTT path: _duck_cancel_and_interrupt already cancelled output, but
    # the forced interrupt must still end the speech handle.
...
def test_non_forced_blocked_when_interruptions_disabled() -> None:
    # half_duplex policy path (allow_interruptions=False, not forced) → no-op.
