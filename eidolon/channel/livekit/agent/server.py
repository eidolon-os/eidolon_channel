...
script, still export the env file path::
    ...
    export EIDOLON_CHANNEL_LIVEKIT_ENV=/path/to/.livekit-channel.env
    EIDOLON_ENV=dev python -m eidolon.channel.livekit.agent.server
"""
...
def _register_plugins() -> None:
    ...
    try:
        from eidolon.channel.livekit.plugins.vad.firered import register_plugin
        register_plugin()
    except ImportError:
        logger.warning("firered pvad plugin not available")

    try:
        from eidolon.channel.livekit.plugins.eot import register_plugin
        register_plugin()
    except ImportError:
...
async def run_agent(ctx, cfg: AgentConfig) -> None:
    ...
    from eidolon.channel.livekit.agent import (
        BatchPipeline,
        SharedStageFactory,
        StreamingPipeline,
    )
    ...
    factory = SharedStageFactory.from_config(
        cfg, prebuilt_vad=prebuilt_vad, livekit_room=room
    )
