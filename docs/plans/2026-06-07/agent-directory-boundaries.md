...
- 新增 `session/interruption.py`。
- 将 soft interrupt fallback timer 移入 `SoftInterruptController`。
- `StreamingPipeline` 保留 `_enter_soft_interrupt`、`_cancel_soft_interrupt`、`_soft_interrupt_timeout_task` 薄 wrapper，兼容既有测试。
- 新增 `SoftInterruptController` 直接单测。

下一步建议：

1. 继续 review `duck/cancel/rollback` 剩余逻辑，不急着整体抽。
