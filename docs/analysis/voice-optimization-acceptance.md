# 语音对话优化：阶段交付与剩余验收

最新入口（2026-09-09）：[冻结基线、复测命令与剩余缺口](voice-baseline-20260909.md)。下文按时间保留各轮结论，不把历史通过率当作当前整体验收结果。

当前结论（2026-09-08）：**按用户确认的范围，本阶段工程交付已收尾；原完整体验目标尚未通过验收。** 本次收尾是停止扩展实现、交付已验证修复并归档剩余项，不是将失败改判为通过。测试数量不作为完成度指标。

交付结果：三项工程修复已提交为 `d110d2b`，整合提交 `942ccc0` 已部署到 Mac 测试环境并完成三模式基础真实 Agent RPC 验证。保留 Channel 的编排职责和 LiveKit 公开接口，未修改 LiveKit SDK。本次仅补交验收记录与[关键结果归档](voice-validation/stage-closeout.json)，不新增运行逻辑。


后续新增目标：用户反馈 mobile 插话时旧音频让声迟缓后，已另行启动接收端让声验证，尚未完成。见文末“mobile 让声目标续验”；不改变以上已关闭阶段的历史交付状态。

测试环境定位：用户明确 Mac host 只是便于执行测试的环境，优化目标仍是通用级联对话能力及三模式体验。后续部署与实测记录不改变这一范围，OPI5 Max 部署后补硬件相关验收。

人工验证补充：用户确认当前 Mac host 已通过 mobile pad 进行了真实对话，对话链路已打通。这是用户报告的实际交互验证；尚未记录该会话的模式、运行版本及延迟测量，因此不推定三模式、六项语义反例或 2500 ms 门槛已通过。Mac + mobile pad 的打通状态与待部署的 OPI5 Max 验收分别记录。

后续日志核对已确认 Mac 存在全双工、真实 Agent RPC 对话，具体版本差异与完成度等待见文末续验；不再将 OPI5 Max 未部署作为 Mac 侧工作的前置条件。

原完整体验目标（2026-09-07 确认，以下保留作为后续验收要求）：沿用下述三个验收关口，先解决影响首段播报的确定性工程缺陷，再评估既有模型／框架能力能否改善条件句和打断语义。生产 RPC 与目标设备验收作为交付依赖明确跟踪。维持 Channel 的编排职责、三种模式及 LiveKit 公开接口，不新增状态机、热词规则或模型提示词变体。候选方案必须同时检查完整句／短答延迟和误打断，不能只让单个反例通过。

以下三个原代码与体验验收关口保持不变。只有三项全部通过，才能宣布完整体验目标达成；本阶段工程交付收尾不代表这些关口已通过，不得通过降低门槛、删除反例或保留 xfail 来宣布达标。

| 关口 | 固定通过条件 | 当前证据 |
| --- | --- | --- |
| 三种模式的功能 | 既有人声画像和三模式回归通过；PTT 只在松键提交、半双工不抢占播报、全双工取消／恢复后不丢失或重复回复；真实语音级联通过 | 最新核心回归 1117 通过；场景总回归 87 通过、6 项既有语义反例未通过，归入下一关。三模式 idle TTS 回归通过；真实级联 15/15 是第二十二批历史证据。本阶段另完成三模式基础 Agent RPC 验证，见续验记录 |
| 完成度与打断语义 | 现有条件句停顿（三种配置）和三个默认意图场景全部通过；判定须有明确上下文，不能把含糊短句固化为全局词规则；既有完整句和短答延迟门槛保持 | `--runxfail` 实跑为 6 失败、5 通过，仍阻断 |
| 默认全双工真实回复 | 原日期更正、费用请求音频，在同样配置和 2500 ms 门槛下，完整提交、恰好一次新回复，并在停声后取得新 TTS 对应的 RTC 音频 | 历史 direct LLM 三次重复为 4/6 通过；最新部署服务 Agent RPC 日期 2788／2842／2580 ms、费用 2956／2895／2694 ms，六次功能验证通过但延迟均失败。两种配置分别记录，仍阻断；第二十二批 2996／6100 ms 历史失败保留 |

直接暴露语义阻断的命令（使用已有模型目录环境）：

```sh
.venv/bin/python -B -m pytest -q --runxfail -p no:cacheprovider -p no:cov \
  eidolon/livekit/tests/scenarios/test_complete_turn_latency.py \
  eidolon/livekit/tests/scenarios/test_human_speaking.py \
  -k 'incomplete_turn or overlap_intent_contract'
```

后续阶段启动时仅处理上述关口的失败及改动引入的回归。冻结新增策略分支、意图提示词变体和新测试框架。补充针对性反例只能用于验证正在修复的根因，不能替代关口。

生产 Agent RPC 的身份缺口已在下述续验中通过在线设备元数据和权威接口解析补齐，三模式基础用例已有真实 RPC 证据；不能用 direct LLM 的通过代替。此前自动预检的特定设备身份返回未指定应答 Companion，该结果不用于否定用户后来确认的 Mac + mobile pad 真实对话。实体设备 AEC、长输出专项验收仍单列；OPI5 Max 待部署后验证。

本轮证据：[第二十二批记录](voice-validation/phase22-validation.json)。历史失败继续保留。


## 本阶段收尾与后续进入条件（2026-09-08）

收尾依据是已验证根因修复、版本整合部署与验收证据归档，不以继续堆叠实验作为进度。最新核心回归 1117 通过、2 skipped、11 xfailed、1 deselected；场景回归强制运行 xfail 后为 87 通过、6 失败。部署后三模式基础用例已通过，但不代表所有说话画像或完整延迟要求通过。关键计数、失败用例和实测结果已存入仓库 JSON；正文中的 `/tmp` 路径是本次原始记录来源，不保证永久可用。

| 剩余项 | 客观状态 | 后续进入条件与验收 |
| --- | --- | --- |
| 完成度与打断语义 | 六项固定反例失败；“你叫啥名儿？”受控全／半双工回放为 3030／3024 ms，超过 1000 ms | 有明确根因与最小修复，或已验证有希望的判定能力后再推进；复跑固定反例，并保持完整句、短答及三模式回归。不增加定向词规则 |
| 真实回复延迟 | 部署服务日期／费用六次均超过 2500 ms；首组主要等待包含有效模型文本及 TTS，不能归因为上下文编排 | 有对应阶段的具体改善方案后重复原音频、原边界和门槛；不能用空文本 chunk、换配置的成功样本或均值掩盖失败 |
| 欢迎语完成等待 | 一次运行及自带重试在用户音频发送前超时，随后原用例复验通过；根因未证实 | 再次复现并取得完成事件链证据后定位，不加绕过等待规则 |
| 目标设备体验 | Mac + mobile pad 已由用户实际对话验证；OPI5 Max 尚未验收 | 部署就绪后完成 AEC、长输出、模式交互及实体播放测量；Mac RTC 帧延迟不能替代扬声器起声 |

本阶段到此关闭，不继续扩展模型清单、提示词变体或编排机制。后续按上表具体条件启动相应工作；原门槛和历史失败持续保留。以下为按时间记录的历史过程，其中“未完成／阻断”指当时及当前仍未通过的完整体验目标。


## 第二十三批补充：如何解释失败

六项语义失败对应四个场景、两类问题，不能计为六个独立架构缺陷。条件句 2.2 秒停顿是句中停顿压力测试；“好的”是介绍过程中附和的语境预期，不是任何语境都禁止打断。日期纠正反例已补齐助手先说错日期的上下文，保留原预期和 xfail，不能用更换上下文消除失败记录。

2500 ms 是第二十批新增真实回放的停声到新音频门槛，不是用户指定的生产 SLA，也不同于开口到可操作转写的预算。第二十三批有效回放：费用 6238 ms，仍不达标；日期完整提交并启动第二次 LLM，但观测结束前没有新 TTS，不能提供首音频延迟或声称成功。费用本轮取消到提交 16 ms，主要等待转移到 TTS；上一轮 2955 ms 的取消到提交耗时仍保留，不能拿本轮单次样本宣布消除。


## 架构收敛复核（2026-09-07）

本次复核覆盖基线 `7204c79` 至 `ef1c313` 的 30 个生产／配置文件。保留已有的会话 EOT 状态隔离、转写 revision／覆盖校正、输出背压与取消、PTT 流关闭、TTS 连接补充及音频处理、原生 LLM 参数透传等修复。输出取消与 SDK endpointing 的适配存在时序耦合，继续限定在原有职责内，不新增状态机、热词表、计时器或提示词分支；不修改 LiveKit SDK。

本次代码修正仅两处：

- **按最终模式构造依赖**：启动入口先解析 participant metadata，再用既有模式策略构造 factory 与 pipeline。半双工／PTT 禁用可选意图客户端；全双工和 SDK 原生 owner 的路由保持。原实现仅在 pipeline 禁用意图消费，工厂却可能按共享配置提前创建模型。六种模式／owner 组合的有效修复前测试为 2 失败、4 通过，失败正是 Channel half duplex／PTT；修复后通过。身份解析失败也不会提前分配供应商客户端。
- **供应商参数按 endpoint 判断**：可选意图模型只对已知 DeepSeek 官方 endpoint 自动关闭思考；未知代理不因模型名带 `deepseek-` 就收到专用参数。复用现有适配器，实际 HTTP 序列化测试覆盖回复／意图两条路径。

**未采用的回退**：曾候选撤销共享配置中显式关闭思考的设置，随后用同一问题、96 completion-token 预算做两次真实模型调用。供应商默认模式消耗 96 tokens、没有可播报文本；显式关闭思考产生 77 字、消耗 47 tokens。因此恢复原配置，本次没有该配置差异。此反例只证明不能直接撤销，不证明复杂回答质量或端到端延迟更优；两次调用不构成稳定延迟统计。原始记录位于本地 `/tmp/voice-consolidation-reasoning.json`。

验证结果：

| 验证 | 实际结果与边界 |
| --- | --- |
| Agent／common／EOT／STT 生命周期／TTS 核心回归 | 1086 通过，19 因沙箱禁止 localhost 监听失败，2 skipped、11 xfailed、1 deselected；原始失败保留于 `/tmp/voice-consolidation-core.xml` |
| 受限网络模块补跑及最终 HTTP 契约测试 | 获准本地监听后，相关两个模块及模型参数模块共 59 通过、1 deselected；与前行有重叠，不累加为独立测试数。记录 `/tmp/voice-consolidation-network-and-contracts.xml` |
| 人声画像／PTT／完整句延迟，强制运行 xfail | 71 通过、6 失败，失败仍为条件句停顿的三种配置，以及附和／继续播报／纠正的三个默认意图场景。记录 `/tmp/voice-consolidation-scenarios.xml` |

本次架构收敛未消除六项既有语义失败，也未重跑真实 RTC，不能覆盖或替代前述 RTC 延迟失败、生产 RPC 鉴权缺口及实体设备验证。整体体验目标仍未通过验收；后续仅围绕上述固定失败关口处理根因，不继续扩大优化范围。


## 本阶段执行记录（2026-09-07）

已落实两项根因修复，均复用现有机制：

- TTS 空闲发送取消自身：`SentenceAggregator` 清空缓冲后取消当前 idle task，遇到异步传输等待就丢失文本。保留执行中任务的归属，正常发送完成或由 `aclose()` 取消；不新增定时器、重试或队列。两个针对性测试修复前失败，修复后通过；三种模式的流式回复均验证完整文本和生成完成前出声。
- 同一产品轮次的旧声学代次裁决污染最终提交：真实 RTC 第三次日期纠正中，generation 3 已确认接管，但 SDK hook 携带 generation 2 的较早 FINAL；原实现提前锁定 generation 2，转写收集结束后仍据旧拒绝裁决否决整轮。改为在既有 settlement 确认产品候选未被替换、组装完整转写后，读取该候选当前 generation 的裁决；删除文本反查代次方法。正反测试验证旧拒绝不阻止新放行、旧放行不越过新拒绝。跨产品候选替换和重复提交仍由原边界防护。

使用同一组端到端回归，在测试进程中恢复 `94a3cc4` 的原方法，得到 4 失败／1 通过：晚 FINAL 的纠正丢回复，以及三种模式 idle 发送丢文本；修正后这 5 项均通过。该复现不改写 SDK 或工作区生产文件。原始记录：`/tmp/voice-stage-end-to-end-before.xml`、`/tmp/voice-stage-merged-vad-after.xml`、`/tmp/voice-stage-idle-three-modes.xml`。

核心回归：1113 通过、2 skipped、11 xfailed、1 deselected（`/tmp/voice-stage-core-final.xml`），未将已知预期失败计为通过。场景总回归以 `--runxfail` 执行，87 通过、6 失败（`/tmp/voice-stage-scenarios-final.xml`）；失败仍为上述条件句三种配置及三个默认意图场景，没有新增场景失败。代码差异检查和 Ruff 严重错误检查通过；与 `94a3cc4` 比较，无新增 Ruff 诊断。

真实 RTC 在裁决归属修复前重复两条原始音频，各三次：日期 2055／1866 ms／一次无新回复，费用 2316／2220／2327 ms。成功样本均通过原 2500 ms 门槛，但那次无新回复阻止整体通过；其 trace 已用于上述确定性回归。该组仅有 TTS idle 修复，不能据此归因历史 6 秒等待已被消除。原始日志与结果：`/tmp/voice-stage-rtc-{1,2,3}/`。

模型候选未落地：FireRed 官方输入格式与当前实现存在差异，但直接替换后条件句仍误判，且“好”“7”低于完成阈值。另对已有多语言权重按原策略运行既有端到端用例，8 通过／2 失败；条件句改善，但“好”的全／半双工延迟分别为 3039／3028 ms，超过原 1000 ms 门槛。因此不改输入、模型或阈值，也不添加短答特例。记录：`/tmp/voice-stage-eot-input-contract.json`、`/tmp/voice-stage-multilingual-candidate.xml`。此前第十二／十三批的远程完成度和 SDK 本地音频 EOT 替换也未通过短答／慢说门槛，本阶段不重复那些替换实验。

裁决修复后的真实 RTC 复验，沿用相同两条音频、默认 `intent_provider=none`、direct LLM 配置及 2500 ms 门槛：

| 用例 | 第一次 | 第二次 | 第三次 |
| --- | --- | --- | --- |
| 我想改一下。不是明天，是后天。 | 2329 ms，通过 | 1966 ms，通过 | 2212 ms，通过 |
| 先别介绍背景。告诉我费用。 | 2092 ms，通过 | 2737 ms，超时 | 3180 ms，超时 |

六次均完整提交并产生一次新回复；每次会话包含最初回答和纠正后的回答，共两次 direct LLM 请求。没有复现旧裁决造成的漏答，但只有 4/6 达到延迟门槛，不能宣称稳定达标。原始记录：`/tmp/voice-stage-rtc-fixed-{1,2,3}/results.json` 及对应 `worker-timeline.jsonl`。

两次费用超时按各自新回复的同一条 trace 时间戳拆分，单位 ms（不混加跨轮次汇总的 `timeline_*` 最大值）：

| 阶段 | 第二次 | 第三次 |
| --- | --- | --- |
| 客户端末个有声帧 → 服务端 VAD 停声 | 617 | 631 |
| VAD 停声 → 完整提交 | 116 | 194 |
| 提交 → LLM 请求开始 | 4 | 3 |
| LLM 请求开始 → 首个文本增量 | 625 | 1580 |
| 首个文本增量 → 首段文本发给 TTS | 457 | 78 |
| TTS 首段发送 → 供应商首音频 | 573 | 332 |
| 供应商首音频 → Channel 首音频标记 | 28 | 27 |
| Channel 首音频标记 → 客户端 RTC 帧 | 317 | 334 |

第二次首段由既有 300 ms idle 机制发出，等待包含文本到达和聚合；第三次主要长在 LLM 首字阶段。两次取消到提交均约 12 ms，TTS 连接获取约 37–40 ms，当前证据不支持再改取消编排或扩充连接池。也不直接缩短 VAD／idle 阈值：前者影响慢说抢答，后者需要验证语音断句质量，且不能解决第三次的首字等待。上述组件阶段含传输和适配耗时，不等于供应商纯推理时间。

本阶段没有改 LiveKit SDK、默认模型、阈值或模式配置。六项既有语义反例尚未修复，真实 RTC 延迟仍未全过；下一步仅接受能同时改善这些固定反例及短答延迟的现有模型／供应商能力，不再通过新增编排分支推进。上表证据来自 direct LLM 和 RTC 接收端，不能替代生产 Agent RPC、实体设备 AEC 或实际扬声器起声测量，也不构成生产 P95；不进入完成状态。

## 生产路径与能力续验（2026-09-07）

本次不新增生产代码，补齐此前可用性未知的链路：

- SDK 云端 EOT：使用已安装 SDK 的 `TurnDetector(version='v1', local_fallback=False)` 和一秒静音做一次受限探测，实际返回 HTTP 401，`is_degraded=True`。SDK 随后返回的默认事件没有计为模型预测成功。已请求现有 Cloud 推理配置路径，不购买服务或默认切换 owner。记录 `/tmp/voice-stage-service-preflight.json` 和同名 `.log`。
- 生产身份：RPC transport ready；本地 LiveKit 有一个在线设备房间。只读获取设备声明的 Owner／Device，再用现有 `preflight_runtime_identity` 查询 Kernel 和 System Data。Owner 默认 Companion 解析成功，设备则明确返回 `has no companion target`。未变更设备分配或向在线设备房间发布音频。记录 `/tmp/voice-stage-runtime-identity-preflight.json`。
- 真实 RPC：用已解析 Owner、独立 agent 名称／端口／房间运行原有严格 benchmark；保留生产 `eidolon_agent` provider、鉴权 resolver 和真实 STT/TTS。测试 worker 的 trace 证明请求实际路由至该 worker。仅关闭 avatar／voiceprint，并重定向诊断音频目录；没有改策略、阈值或验收断言。

| 原有用例 | 实际结果 | 证据边界 |
| --- | --- | --- |
| `fd_gate_normal_turn_replies_001` | 通过；1 次正式 RPC 请求及回复，666880 bytes RTC 音频 | 全双工正常输入；停声后有声 RTC 帧 3536 ms，不是 2500 ms 延迟通过 |
| `streaming_three_short_rounds_commit_each_001` | 通过；连续 3 次独立正式 RPC 请求完成并有 TTS，3056640 bytes RTC 音频 | 半双工连续轮次；设备状态／播放确认仍由客户端模拟；末轮停声后有声帧 2910 ms，不代表三轮的延迟分布 |
| `phase_a_ptt_normal_release_commits_001` | 通过；松键提交、真实 RPC 与 TTS，674560 bytes RTC 音频 | PTT 按钮边界由客户端模拟；release 后有声帧 2876 ms，不是实体设备验证 |

上述三项 `real_call_verified=True`，无用例错误。trace 中均存在 `brain_rpc.provider=eidolon_agent_rpc`、独立 request_id 和请求／首字时间戳；半双工三轮并非重复计算同一请求。**计数校正**：上述数字只证明正式回复请求数。随后核对同一批 worker 的 `EidolonAgentSession.start_turn` 日志，实际总数为全双工 3、半双工 6、PTT 1；流式模式存在未进入正式回复 trace 的额外投机请求，见下节修正。报告分别位于 `/tmp/voice-stage-rpc/{owner-normal,owner-half_duplex,owner-ptt}/barge_in_e2e_ab/channel/livekit_room/`。入口复用 `scripts/bench_barge_in_e2e_ab.py`，临时调用参数见 `/tmp/voice_stage_rpc.py`。这补齐了三模式基础生产 RPC 功能证据，没有替代六项语义反例、默认日期／费用的延迟门槛或实体设备验收。

延迟核查：全双工这次请求的 TTS 首段发送到供应商首音频约 1145 ms。SDK `AudioEmitter` 的 200 ms 默认帧设置使用 progressive 输出，不能仅凭该数值断言固定增加 200 ms 等待。另复用现有 TTS stage 合成两句原测试文本，输出音频从起点到首个 RMS≥120 的 20 ms 帧分别为 120／60 ms（`/tmp/voice-stage-tts-onset.json`）；低音量起声本身也计入 RTC 有声帧指标，不能把 Channel 首音频标记到客户端有声帧的全部耗时归为网络／缓冲，更不能直接裁掉这些帧。没有据此新增音频裁剪或调小 SDK 缓冲。安装的 LiveKit Agents 1.7.1 共 207 个带校验值的 Python 文件全部匹配 RECORD。

现有模型实验也已去重核对：第十六批 Smart Turn v3.2 为离线 16/24、真实 STT/VAD 与 SDK 回放 1/6 通过，已经拒绝，本轮未重新下载或重跑。官方 [LiveKit 自适应打断说明](https://docs.livekit.io/agents/logic/turns/adaptive-interruption-handling/)提供了云端声学附和判断路径，但当前访问探测失败，且其公开能力说明不能证明本项目反例会通过。仍需有效的现成推理配置进行验收，实体设备则需明确测试目标及应答 Companion；未降低任何结束条件。

## 预生成关闭仍发起投机 RPC：修正与验证（2026-09-07）

检查是否能复用现有预生成能力改善延迟时，发现 `preemptive.enabled=False` 只传给 SDK，Channel 却无条件绑定 interim RPC 预热钩子。`PreemptiveWarmer` 会发起 `StartTurn(speculative=True)`，该请求的输出被丢弃，也不进入正式回复的 provider trace。前节真实日志的额外 2／3 次请求与这条路径一致；不能把正式回复计数当作服务调用总数。

修正仅在既有转写处理器的构造处，按同一个 `preemptive.enabled` 决定是否绑定现有预热回调。默认关闭时不再发送投机 RPC，显式启用保持原行为。未增加开关、任务、队列或缓存；原有 `StartTurn` 日志增加 `speculative` 字段，便于直接核对实际调用量。同步纠正把 RPC 预热和 SDK 可复用预生成混称为同一个过程的说明。本次没有启用 SDK 预生成作为延迟捷径；RPC adapter 的正式 `chat()` 仍按正式 turn 发送，不能仅凭框架“预生成”名称推断服务端投机副作用契约已成立。

复用生产 Pipeline、实际 LiveKit AgentSession 和 RPC adapter、已有本地 gRPC 服务及 ASR/TTS 测试替身，对全／半双工和开关启闭四种组合直接检查 protobuf `StartTurn.speculative`：修正前 2 失败／2 通过，失败均为关闭后仍发送投机请求；修正后 4 项通过，完整转写只形成一次正式回复，受控停声到音频不超过原 1000 ms 门槛。相关模块合计 47 通过。证据：`/tmp/voice-stage-preemptive-before.xml`、`/tmp/voice-stage-preemptive-after.xml`。

核心回归为 **1117 通过、2 skipped、11 xfailed、1 deselected**（`/tmp/voice-stage-core-preemptive-final.xml`）。随后增加的 RPC 日志字段在真实复验中执行并核对；Ruff 严重错误和差异检查通过。没有重复跑实现路径未变化的整套人声画像，也没有宣称原六项语义失败消失。

同一已解析 Owner、同一现有用例和独立 worker 的生产复验：

| 用例 | 请求计数及功能 | 本次有声 RTC 帧延迟 |
| --- | --- | --- |
| 全双工正常一轮 | 通过；总 `StartTurn` 3 → 1，唯一请求 `speculative=False`，624000 bytes 音频 | 停声后 2143 ms |
| 半双工连续三轮 | 通过；总 `StartTurn` 6 → 3，全部 `speculative=False`，3157120 bytes 音频 | 末轮停声后 2476 ms |

报告与原始日志：`/tmp/voice-stage-rpc/{preemptive-off-full-duplex,preemptive-off-half-duplex}/barge_in_e2e_ab/channel/`。两项 `real_call_verified=True`；实际额外请求消失已证实，但供应商和回复内容存在波动，单次对照不能将延迟差值归因于本修正，也不替代日期／费用两条原 2500 ms 关口。PTT 不经过此次修改的 interim 钩子；其既有真实 RPC 验证和本轮核心回归保留。

依赖再核对：没有新增显式 Cloud 推理配置，设备权威预检仍为 `has no companion target`（`/tmp/voice-stage-dependency-recheck.json`）。未重复请求已拒绝的云端推理、改写设备分配或修改 SDK。六项语义和原 RTC 延迟关口仍阻断整体完成，目标保持不变。

## 现有百炼模型能力筛选（2026-09-07）

使用现有百炼配置成功查询可见模型，确认 `qwen3.8-flash` 和 `qwen-flash` 可实际调用。复用现有 OpenAI-compatible adapter、打断分类器及第十二批固定完成度提示词；仅在临时测试配置中指定模型和 `enable_thinking=False`。单并发、先预热、每项仍限 1500 ms，无重试、提示词变体、阈值变更或生产配置改动。

| 候选 | 完成度：通过／总数 | 超时／其他失败 | 打断意图：通过／总数 | 超时／其他失败 |
| --- | --- | --- | --- | --- |
| Qwen 3.8 Flash | 22/58 | 33/3 | 6/12 | 6/0 |
| Qwen Flash | 26/58 | 23/9 | 9/12 | 2/1 |

完成度沿用 24 对未完成／完整句，并包含上下文短答、数字、纠正和停止请求；意图沿用既有 12 条模型重叠场景文本。后者使用该组既有的通用介绍上下文，不替代六项正式关口中明确说错日期的纠正上下文。除超时外，存在完整请求被判未完成，以及输出非约定标签的失败。未放宽标签解析来隐藏这些问题。

两候选均未达到进入端到端替换验证的条件，因此不接入生产，也不把上述文本级筛选计作 RTC 或六项反例通过。结果仅适用于本次接入路径、提示词与截止时间，不构成模型能力排名或稳定延迟统计。模型访问及逐项原始记录：`/tmp/voice-stage-bailian-model-access.json`、`/tmp/voice-stage-qwen-semantics.json`；复用脚本 `/tmp/voice_stage_qwen_semantics.py`。

本次没有新的生产修改，保留前述三个已验证的工程根因修复。剩余语义缺口未补齐，不能用继续增加编排分支或定向词规则代替满足要求的判定能力；原 RTC 延迟关口和实体设备验证仍未完成。暂不扩展候选模型清单或重复已拒绝实验，有新的可用能力或配置后再按原关口验证。

## 延迟归因与依赖复核（2026-09-07）

沿用原费用失败的 worker HTTP 调试日志，进一步核对连接阶段，没有另跑一次随机样本替换原失败：第二次 TCP/TLS 从 `20:58:59.612` 到 `.645`，约 33 ms；第三次从 `21:00:07.789` 到 `.827`，约 38 ms。两次 HTTP 响应头分别在 `.721` 和 `.904` 到达。第三次对应首个文本增量仍约在 `21:00:09.369`，响应头后约 1.46 秒；这是流式响应内容等待，不能仅凭客户端日志进一步分离服务端排队、生成和传输。现有证据不支持通过另造 HTTP 连接池解决主要超时。

第二次 TTS 首段在 `20:59:00.694` 由现有 idle 定时器发送，内容是“费用取决于具体实现方式”；下一段标点／文本在 `.970` 才发送。该记录与此前聚合等待归因一致，没有发现新计时器失效。缩短 idle 会改变断句取舍，且无法解决第三次的首字等待，因此不以调参替代修复。这里只补齐归因证据，未宣称 RTC 门槛通过。

配置 schema 中仍将 SDK 可复用预生成与 RPC 临时预热混为一谈的注释已同步更正，无运行行为变更。最后一次权威设备预检仍返回 `has no companion target`，显式 Cloud 推理配置仍未提供（`/tmp/voice-stage-dependency-final-recheck.json`）；没有重复调用已返回 401 的推理服务。该设备依赖已在连续阶段复核中保持不变，现有语义候选均未通过准入。剩余验收需要可验证的判定能力以及明确的实体测试目标／Companion，不能据现有结果宣布完成。

## Mac 实际对话续验与版本整合

用户确认 Mac + mobile pad 对话打通后，检查现有生产日志与源码：`livekit-agent-2026-09-07.log` 的 `23:20:17.175` 明确记录 `interaction_mode=full_duplex`；同一设备房间的三条回复 timeline 均含 `brain_rpc.provider=eidolon_agent_rpc`。这补充了实际链路证据，先前该身份的 Companion 预检失败不再作为整条 Mac 链路不可用的结论。

生产 worker 堆栈使用 `/Users/manson/ai/eidolon/eidolon_channel`。当时该源码目录 HEAD 为 `5e46807`，不包含 `d110d2b`；不能把这次真实对话当作已部署优化分支的验收。磁盘 HEAD 本身也不足以证明运行中进程的准确 commit。已将生产分支截至 `5e46807` 的 Manifest 契约及启动／退出通知修复合入优化工作分支，自动合并无冲突，保留双方行为；没有改写生产源码或重启服务。

整合验证覆盖 Channel Provider、会话 metadata／模式解析、启动失败通知及意图模型配置：首轮 200 通过，2 项因沙箱禁止 localhost 监听失败；允许本地监听后，对应 HTTP 模块 8 通过，其中 6 项与首轮重叠，不累加测试数。记录 `/tmp/voice-mac-integration.xml`、`/tmp/voice-mac-integration-http.xml`。差异检查通过。

真实样本“你叫啥名儿？”：服务端 VAD 于 `23:20:52.018` 停声，FINAL 于 `.187` 到达；EOT 得分 0.247，低于 0.5 完成阈值，直到 `23:20:54.523` 才进入回复。该轮 timeline 停声到提交 2501 ms，提交到 Channel 首音频 1180 ms。这是服务端阶段指标，不是 pad 扬声器起声或原 RTC 2500 ms 验收。

在整合后的工作分支直接复用既有 `test_short_answer_replies_promptly`，仅传入这条实际文本，全／半双工分别测得 3030／3024 ms，均超过原 1000 ms 门槛；本地 EOT 得分同为 0.246676。此受控回放使用真实 EOT 与 SDK、模拟 ASR/TTS，不冒充真机重测；记录 `/tmp/voice-mac-colloquial-replay.json`。结果确认该完整口语问句的等待属于完成度误判，未通过改词、降阈值或新增取消分支绕过。原六项反例仍是固定关口，这条实际样本作为同一模型能力缺口的补充证据。

## 通用链路部署后验收（2026-09-08）

用户确认继续后，将 Mac 测试环境的 Channel 源码从 `5e46807` 快进到整合提交 `942ccc0`，通过现有 eidolond `system.service.restart` 接口重启 Channel，未绕过原生命周期管理。重启后服务为 ready，健康接口与 worker 注册接口均为 HTTP 200，worker 注册名为 `eidolon`，进程 PID 从 84655 变为 2349。记录 `/tmp/voice-mac-deploy-before.json`、`/tmp/voice-mac-restart-result.json`、`/tmp/voice-mac-deploy-after.json`。部署后观察到运行源码目录出现其他工作的未提交修改，未覆盖或纳入本次提交；这些验收结果描述实际运行服务，不能扩展为该目录后续任意未提交版本的保证。

复用既有 benchmark 与身份解析，在独立测试房间直接请求已部署服务；没有另起替代 worker，也未向用户的设备房间发布音频。以下经过真实 ASR、Agent RPC、TTS 与 RTC；客户端按钮／播放状态仍由原测试模拟，末端指标是 RTC 有声帧，不是实体扬声器起声。

| 既有用例 | 结果 | 正式 RPC 请求数 | 本次停声／松键到有声 RTC 帧 |
| --- | --- | --- | --- |
| 全双工正常一轮 | 复验通过 | 1 | 2362 ms |
| 半双工连续三轮 | 通过 | 3 | 末轮 1779 ms |
| PTT 松键提交 | 通过 | 1 | 1804 ms |

三项均 `real_call_verified=True`。原始报告位于 `/tmp/voice-stage-rpc/{deployed-full-duplex-recheck,deployed-half-duplex,deployed-ptt}/barge_in_e2e_ab/channel/livekit_room/`。全双工首轮及 runner 自带的一次重试失败记录仍位于 `deployed-full-duplex`：欢迎语音频已收到，但没有欢迎语最终转写，测试在发送用户音频之前等待超时。因此不能计作输入后的漏答，也不能隐去该接入不稳定现象。随后通过 SDK 公开 text-stream 接口的独立诊断观察到完整欢迎语、legacy 与 stream 两条转写以及真实回复（`/tmp/voice-transcription-probe.json`），再按原严格用例复验通过。尚未把偶发现象归因为特定播放时序缺陷；没有修改 SDK 或添加绕过等待的规则。SDK 207 个受 RECORD 校验的 Python 文件仍全部匹配。

继续使用日期纠正／费用请求的两条原始音频，各重复三次，保留完整提交、恰好一次新回复及 2500 ms 门槛。此次走部署服务的真实 Agent RPC 与实际配置，与此前 direct LLM 结果分开记录：

| 原音频 | 第一次 | 第二次 | 第三次 |
| --- | --- | --- | --- |
| 我想改一下。不是明天，是后天。 | 2788 ms | 2842 ms | 2580 ms |
| 先别介绍背景。告诉我费用。 | 2956 ms | 2895 ms | 2694 ms |

六次均完整提交、每会话产生两次正式 RPC（原回复及一次纠正后的新回复），真实调用验证通过；**六次均未通过 2500 ms 延迟门槛**，没有用新样本替换先前 direct LLM 的 4/6 结果。原始结果和逐轮 timeline 位于 `/tmp/voice-stage-rpc/deployed-corrections/barge_in_e2e_ab/channel/livekit_room/repeat-{00,01,02}/`，摘要 `/tmp/voice-deployed-corrections-summary.json`；入口 `/tmp/voice_deployed_corrections.py` 复用原 runner 和先前的边界计算方法。

第一组按各自新回复 trace 拆分：日期／费用的服务端停声到提交为 74／82 ms，RPC 发送到有效文本为 1213／1389 ms，有效文本到首段 TTS 发送为 299／226 ms，TTS 发送到供应商首音频为 309／341 ms。其余时间含客户端停声到 VAD、音频输出与传输，不把上述四段误称全部延迟。按 request_id 核对 Agent 自身日志，上下文编译约 32／27 ms，模型有效文本 TTFT 为 1176／1357 ms；首个原始 chunk 更早，但没有可播报文本。上下文准备并非这两次主要等待，现有证据不支持重写该编排。此处沿用服务日志的指标含义，不把适配器的 `connect_ms` 当成纯 TCP/TLS 时间。

本轮完成版本部署、三模式基础验收和原音频的生产 RPC 重复测量，没有新增平台专用实现或变更模型／阈值。六项既有语义反例、口语完成度误判及低延迟缺口仍未补齐，整体目标未完成。


## mobile 让声目标续验（2026-09-08）

目标是全双工用户实际起声后及时、可恢复地让声，并保持三模式原语义。受控接收端目标为旧音频衰减至少 12 dB、P95 ≤300 ms；误触发后恢复原回复，不丢字、不重复。服务端 VAD 起点、duck 请求、RTC 接收端衰减及实体扬声器必须分别计量。用户本次明确暂不方便 mobile 外放验收，先做自动化验证；该依赖不能冒充通过。

本节首轮仅修改既有 benchmark 测量：将 `interrupt_started_at` 的 `duck_silence` 标签纠正为 `duck_requested`；记录每段输入首个有声发布帧，补充接收端 RMS 衰减观察值。指标要求连续 100 ms 满足 12 dB 衰减，缺失、过旧或已静音的基线，以及收包空洞不计成功。对自然语音，停顿也能导致音量下降，因此此值不单独证明 duck 生效，不作为新的自动通过门槛；连续测试音用于隔离输出控制。沿用原 benchmark 的两模块回归 170 通过，Ruff 严重错误和差异检查通过。

实际 LiveKit Agents 1.7.1 队列隔离复现：预先快速提交 2 秒连续 440 Hz 测试音，收到声音 150 ms 后发起 duck，并保持裁决未决 2.2 秒。RTC 测量如下，均从服务端 duck 请求起算，**不含上行、VAD、实体设备播放**：

| 路径 | 三次接收端衰减延迟 | 暂停时被清除的待播队列 | 结论 |
| --- | --- | --- | --- |
| 当前 Channel duck | 1908.6／1888.3／1888.9 ms | 0 ms | 停止后续送入音频不能阻止 SDK 已排队旧音频继续播放 |
| 直接调用 SDK 公开 pause | 108.5／107.8／109.8 ms | 208.7／210.0／203.9 ms | 有约 200 ms 未播放音频被丢弃，不作为无损恢复方案上线 |
| 排完 AudioSource 队列后再暂停的审阅草案 | 312.1／307.7／369.4 ms | 本轮未观察到清队列 | 仅输出阶段已出现超过 300 ms 的样本，尚未包含上行/VAD，未采用；未证明逐字恢复完整性 |
| 在上述草案中将原生队列预算设为 60 ms | 189.2／212.8／189.1 ms | 本轮未观察到清队列 | 历史诊断候选；用户已明确不修改 SDK，此方向撤回。未证明整体 P95、抖动容忍或逐字恢复 |

后两行仅在独立诊断进程中替换方法验证草案，没有写入 SDK 安装文件、工作区生产实现或部署服务。SDK 207 个 Python 文件仍与 RECORD 全部匹配。所核对的[上游 main 输出实现](https://raw.githubusercontent.com/livekit/agents/main/livekit-agents/livekit/agents/voice/room_io/_output.py)同样存在暂停时清除 AudioSource 队列的行为，因此未盲目升级依赖。此结论不宣称所有 SDK 版本都没有可行方案。

既有真实对话用例（原 ASR / Agent RPC / TTS / RTC）：附和恢复功能通过，服务端 speech-start 到 duck 请求约 0.25 ms，而输入首个有声发布帧到接收端衰减观察值为 679 ms；服务端恢复耗时 1444 ms，超过原 900 ms 门槛，整体失败。停止播报用例在欢迎语完成等待阶段超时，尚未发送用户音频，不能当成打断用例通过或输入丢失；其自动重试失败记录保留。自然 TTS 的 679 ms 是观察值，不能与连续音的因果隔离实验混作同一组统计，更不能据单次结果宣布 P95。

完整证据摘要：[接收端让声验证](voice-validation/duck-receiver-validation.json)。当前结论：测量盲区已补，队列尾音已复现，但**没有满足约束的生产修复，目标未达成**。现有 800 ms 冷却窗口未改，尚未确认其为这组首次 duck 的根因；不以删除冷却或压低 EOT 阈值掩盖队列问题。

用户随后明确不修改第三方 livekit-agents，以免承担持续补丁维护成本；SDK 修改建议已撤回。上表草案仅保留为历史诊断证据。后续边界是核查 Channel 自身的帧提交和缓存恢复，修复有复现证据的应用侧问题；不修改 SDK 安装文件或私有字段，不增加第二套播放时钟／音频队列，不改变 EOT 门槛或冷却窗口。


## Channel 输出内容保全与送帧核查（2026-09-08）

本次沿“不修改第三方 SDK”的边界确认并修复两处应用侧缺陷，未新增时钟、队列、热词或编排分支：

- 淡出结束点落在音频帧中间时，原实现会把余下 PCM 静音送走。200 ms 帧配 30 ms 淡出会损失约 170 ms 内容。现在只发送淡出所需前缀，其余原样交回既有状态、代次、缓存容量检查；同时按每声道样本推进淡出，并只把送出的前缀计入输出时长估计。
- 超时恢复原回复时，原策略将尚未播放的缓存视为“过期”并丢弃。原真实 RTC 日志明确丢掉 10 帧、1.97 秒内容，但后续音频仍继续播放；丢缓存并没有消除恢复说话这一动作。现在证据期限耗尽和无转写的 VAD idle deadline 都复用保留缓存的 rollback；确认打断的 cancel、PTT 显式抢占及已有丢弃控制保持原语义。决定何时恢复的期限没有改变，因此超时后与仍在说话的用户重叠这一风险也没有宣称解决。

因果验证：只在测试进程恢复旧 Channel 淡出方法，三个附和场景均因原始 PCM 被静音而失败；默认真实 EOT 的“好。”超时恢复场景在第二项修正前仅返回 24320 个样本，原始 TTS 有 43200 个样本（16 kHz），少约 1.18 秒。测试只允许 SDK 明确追加的 10 ms 全零结束标记，保留原始内容长度和静音丢失断言；组件测试另外验证不同帧切分下的逐样本结果以及取消／清空／重置／恢复并发。

真实 RTC 使用原 ASR、Agent RPC、TTS 和既有附和用例。修正后记录 `drop_buffered=False`、10 帧全部 drain、丢弃计数为零。该次仍因恢复耗时 6655 ms 超过原 900 ms 门槛而失败；接收端衰减观察值 1207 ms。前一次为 6554／174 ms，供应商输出和自然语音停顿不同，两次不构成因果延迟 A/B，也不是 P95。复验含一次欢迎语阶段失败后的 runner 原有重试，没有删除失败记录。测试结束会打断剩余播放，RTC 缓存计数不能替代整句实体听感验收。

工程核查也确认：恢复时 1.97 秒 Channel 缓存在约 14.9 ms 内被下游接收；旧丢缓存路径在恢复后约 111 ms 内仍送入了后续约 13.17 秒音频。积压来源不只包含 Channel 暂存内容，尚未发送到 Channel 的上游音频也会快速涌入。因此仅缩小 Channel 缓存或丢弃恢复缓存不能建立播放延迟上限；没有据此加第二套 pacing 或改 SDK 私有队列。上述计时来自临时 worker 对 Channel 方法的观测，capture 返回表示入队，不表示扬声器已播放，也没有测出完整下游队列深度。

最终版本回归：输出与策略 151 项通过，真实框架场景 23 项通过，三模式人类说话／PTT／完整句延迟／附和恢复 79 项通过、6 项既有语义反例仍为严格预期失败；未降低门槛。Ruff 严重错误和差异检查通过。验收证据见 [Channel 输出内容保全验证](voice-validation/channel-output-preservation.json)。本次修复解决两处内容丢失；低延迟让声、六项既有语义缺口及实体设备目标仍未完成。未部署到现有 Mac 服务；Mac 只作为自动化测试环境。LiveKit Agents 1.7.1 的 207 个受 RECORD 校验的 Python 文件仍全部匹配。


## 优先级执行：能力边界核验（2026-09-08）

本轮未新增运行逻辑；按先有限核查 HOLD、再独立检查快速让声、再回到回复延迟的顺序推进。证据归档：[优先级执行与能力核验](voice-validation/priority-capability-validation.json)。

- **6 秒等待**：纯 `InterruptDecider` 输入 FINAL、VAD idle、EOT 0.668，直接得到 HOLD；deadline 仍等待裁决。独立的 0.19／0.71 FINAL 输入分别恢复／取消，说明中间分数区间没有足够裁决依据。复用原默认超时内容场景，观察 Channel 方法：内容断言通过，但 duck 到 unduck 为 6702 ms；期间没有进入产品 completed-turn hook。SDK 在 Channel 尚未确认打断时跳过不可打断语音后的新回复符合接管契约，不能据此认定 SDK 漏回调。未以调用 hook、放开 SDK 自动打断或缩短期限强行结束等待。
- **快速让声**：当前业务 RTC 连接拒绝后，用本机已安装 LiveKit 1.11.0 启动独立回环实例，测试结束关闭。全程通过公开 AgentSession／RoomOptions／AudioOutput 接口，未引用或改写 SDK 私有对象。预先提交 400 ms 连续音，两次 duck 到 RTC 衰减为 308.8／309.3 ms；预先提交 2 秒时为 1909.2／1889.8 ms。四次在 2.2 秒未决窗口内均没有播放进度事件，`wait_for_playout()` 也未完成，直到后续恢复并结束片段才完成。公开事件不是持续的消费确认；不能据此直接构建有界送帧反馈。400 ms 是实验输入量，不是已实现的队列上限，也没有达到整链路 300 ms P95。
- **完整请求首回复**：补齐原六次真实日期／费用 trace 的逐请求阶段分析。服务端停声到提交 40.0～136.1 ms，提交到 RPC 1.9～11.3 ms，RPC 到有效回答文本 645.1～1388.6 ms，文本到首段 TTS 189.3～341.3 ms，TTS 到供应商首音频 308.7～536.9 ms。没有新运行替换原失败；这些阶段不包含全部上下行延迟，不支持重写提交编排或用空文本 chunk 代替有效首字。

业务复验的四个既有回环端口（7880／45051／8083／8084）在获准网络检查后均返回 ECONNREFUSED，已向用户询问应使用的测试环境或现有启动入口；未重启业务服务。公开接口隔离测试不能代替真实 Agent RPC、mobile 扬声器或 OPI5 Max 验收。LiveKit Agents 1.7.1 的 207 个受 RECORD 校验 Python 文件仍全部匹配；保留前轮两项内容修复，完整体验目标未通过。

## 环境恢复后的业务回归与接收端内容核验（2026-09-08）

已找到并使用现有 Ops 统一入口 `eidolon mac start` 恢复测试环境，健康检查与 `app-ready` 均通过，不再要求用户手动启动四个服务。当前工作区通过既有 managed-worker benchmark 在独立名称、端口与房间中测试；未覆盖 Mac 原源码目录，未把本轮工作区改动部署到业务 worker。

本轮发现并修正一处 **测试前置条件误判**：欢迎语首音频晚于既有 6 秒等待时，runner 把尚未起声当作没有欢迎语，先发送普通问题，再把迟到的欢迎语当作问题的回答进行打断。初次停止用例原报告虽为通过，但其首次正式 RPC 发生在停止输入之后，裁决文本为上一条问题的迟到 interim，因此该样本不计作有效通过。现在复用运行时 `resolve_session_intent`／`resolve_welcome_text` 推导是否应有欢迎语；缺失预期音频即记录准备失败，未延长超时、增加重试或修改运行时策略。两个 benchmark 模块 172 项通过，包括有／无欢迎语及主动会话的原有契约。

真实 ASR、Agent RPC、TTS、RTC 回归结果：

| 既有用例 | 有效结果 | 本轮指标与限制 |
| --- | --- | --- |
| 全双工普通问题 | 功能通过，1 次正式 RPC | 停声到有声 RTC 帧 3337 ms；该功能用例未设置 2500 ms 断言，不能据通过标签称低延迟达标 |
| 半双工同一短问重复三轮 | 功能通过，3 次正式 RPC | 末轮停声到有声帧 2402 ms；不代表三轮延迟分布 |
| PTT 松键提交 | 功能通过，1 次正式 RPC | 松键到有声帧 2626 ms；客户端按钮／播放确认仍为模拟 |
| 全双工“停一下” | 修正前置条件后通过；首次回答与停止后回答共 2 次正式 RPC | 原问题完整 FINAL 后已产生正式回复；取消依据为“停一下”，服务端 speech-start 到取消约 521 ms |
| 全双工“好。”附和 | 恢复功能通过，延迟失败；仅 1 次正式 RPC | 默认 EOT 约 0.668 仍 HOLD，speech-start 到恢复 6650 ms，超过原 900 ms；10 帧约 1.97 秒缓存保留并恢复 |

最终停止／附和两项均无重试且真实调用验证通过。自然 TTS 下输入首个有声发布帧到 RTC 衰减观察值分别为 226／1998 ms，不能把语音自然停顿视为 duck 的因果效果或据此报告 P95。第一次附和的欢迎语等待失败与后续无 agent 的重试失败记录保留；它们不计入语义通过数。各批日志按 `StartTurn` 核对，没有投机请求。此前日期／费用六次 2500 ms 失败及六项固定语义反例保持未通过，本轮没有用普通问题的测量替代它们。

接收端内容验证补齐了此前只检查 Channel 缓存计数的边界：用 20 段各 100 ms、频率不同且顺序固定的测试音，按 200 ms 帧送入原 OutputController 与公开 AgentSession 输出，真实 RTC 接收端按 20 ms 帧识别。连续播放基线与淡出／暂存／恢复均收到全部 20 段且顺序正确；恢复路径暂存 1570 ms、drain 8 帧、丢弃计数为零。显式丢缓存对照只收到前 5 段，检测确实失败。每段允许 60～140 ms 以容纳淡变、编解码和帧边界，此结果证明本次未观察到整段缺失或重复，不等于逐样本无损、逐字听感、实体扬声器或快速让声达标。

本轮未新增生产运行逻辑、热词、播放时钟或音频队列；SDK 1.7.1 的 207 个 Python 文件仍与 RECORD 一致。证据归档：[环境恢复后的回归](voice-validation/restored-environment-regression.json)。两项应用侧内容修复保留；默认附和裁决、接收端快速让声和完整请求低延迟仍是未完成项。

## 按机制审查等待与恢复，拒绝按 FINAL 快捷恢复（2026-09-08）

用户要求泛化处理后，本轮先审查等待的证据含义。ASR 句段定稿、VAD 停声、用户话轮结束和打断意图是不同事实；不能因为已有 FINAL 就推断没有后续输入，也不能把 EOT 中间分数解释为附和。

| 机制／场景 | 当前证据与应保持的边界 |
| --- | --- |
| 用户仍在说，或分句后短暂停顿 | FINAL 可能只覆盖一个句段；不得把它单独作为恢复或提交依据 |
| 停声后转写尚未到齐 | 等待的是外部转写证据；不能将无文本误称为已确认噪声 |
| 转写已定稿，配置的意图请求仍在执行 | 等待的是现有分类请求，结果必须绑定当前候选、声学代次和转写版本；不能再造一个分类器或计时器 |
| 默认策略收到 FINAL，但完成度／意图仍不明确 | 当前没有待返回的意图请求；6 秒窗口仍允许后续证据，不能把这段时间描述成模型推理耗时，也不能凭定稿直接判附和 |
| 已取得可执行的停止、接管或恢复证据 | 沿现有唯一裁决入口执行，不能继续等满兜底时间 |
| 证据预算耗尽 | 这是不确定状态下的兜底恢复，不等于正确识别了附和；恢复内容完整性与恢复时机分开验收 |
| Channel 已执行 duck／resume，但接收端未变化 | 检查下游排队与真实音频，不用裁决时间冒充实际播放时间 |

对“默认策略已有 FINAL、VAD 已停就立即恢复”的假设做了进程内对照，只替换 Channel 纯 decider 的返回，未修改 SDK 或工作区运行实现。复用真实 AgentSession、生产 Pipeline、原转写拼接和输出控制，新增日期、人数、英文目的地三种连续修改表达，各覆盖 FINAL 在 VAD 停声前／后到达；中间停顿 400 ms，仍在现有 800 ms 合并窗口内。EOT 边界注入前段 0.4、后段 0.95，明确隔离模型准确率，不将其计作真实模型或 RTC 验收。

当前实现六项均通过：停顿中保持让声，后续完整文本只形成一次新回复。快捷恢复草案六项均在停顿中恢复原播放，违反连续表达的让声断言；最终文本仍完整提交，没有观察到丢字或重复提交，不能夸大失败类型。因此拒绝这项草案，也不将其转换为短词、语言或特定分数的例外规则。原始对照与最终回归见 [机制验证](voice-validation/interruption-mechanism-validation.json)。

最终时序场景、候选归属和超时处理三个模块合计 59 项通过（含新增六项）；JUnit 属性输出产生六条格式兼容提示，结果和属性均已归档，无测试失败。检查了与前轮证据中的三个生产修复文件哈希一致，未把进程内草案遗留到生产实现。Ruff 严重错误和差异空白检查通过。

本次结论限定为否决“FINAL 即可恢复”，不构成保留 6 秒数值本身的充分依据。900 ms 是既有短附和回归预算，历史上从 800 ms 调整而来；尚无证据支持把它作为通用产品 SLA。后续顺序保持为实际播放端让声、完整输入后的回复延迟、含糊短反馈恢复；对最后一项必须同时验证暂停续说、迟到修订、明确接管、模型超时和恢复完整性，不能让单个“好。”反复测试占据主线。本轮未修改策略阈值、期限、模式所有权或生产配置，也未声称六项原语义反例或 6 秒等待已修复。


### 2026-09-08 mobile 外放：OPI5 Max 慢说续句失败

本轮用户反馈：公园活动正常回复、切换室内活动正常打断；“人数方面……改成两个人”未打断，旧回复继续。ADB 网络连接与远端网卡核实实际服务是 orangepi5-max（192.168.100.19），不是 Mac。部署进程启动于 20:30:04，入口 /opt/eidolon/current/eidolon_channel/.venv/bin/python；具体 revision 尚未核实，不计作本工作区通过验收。

证据见 `voice-validation/mobile-opi5max-pause-feedback.json`。两段文本均进入 ASR。第一段低 EOT（约 0.0005；重试约 0.0034）在 sentence FINAL / VAD idle 时触发 `final_terminal_eot_low` rollback。后续开口分别在 rollback 后约 0.41 / 0.45 秒，日志明确显示 800ms cooldown 跳过 duck；后续文本走 `attention_blocked`，没有正式提交。第一次前缀候选被后续开口替换；另一次成功合并成“人数方面。改成两个人。”，仍未提交直到 session close。因此既有恢复裁决过早的问题，也有恢复后续句重新准入的问题；不能归因为 ASR 漏字或简单扩大拼接窗口。

这暴露了此前脚本测试的边界：停顿续句测试注入 0.4 / 0.95 的 EOT 分数，没有覆盖实际低 EOT 触发恢复的分支。59 项通过不证明这条真机路径正常。下一步应复现低 EOT + 句级 FINAL + 停顿续说在冷却窗口内外的通用机制，并覆盖真实附和、噪音和重复开口；修正应沿现有 Channel 裁决与候选生命周期，不增加热词规则、第二套编排或第三方 SDK 修改。

用户反馈为定性听感，没有设备扬声器时间戳，不报告物理停止延迟或 P95。成功室内活动轮服务端 VAD onset 到 cancel 约 493ms；此值不等于耳朵听到的停声延迟。公园问题日志另有一次早先 LLM error，不能将所有尝试均计为通过。

### 暂停续说修复与自动化回归（2026-09-08）

证据归档：[暂停续说恢复验证](voice-validation/pause-continuation-recovery.json)。本轮修复的是“播放暂时恢复后，尚未裁决的用户话轮不能继续收集和处理证据”的生命周期问题。低 EOT 与句级 FINAL 不再直接作终局拒绝；新增非终局 RESUME 动作，在既有继续说话保护窗口后允许恢复输出，同时保留同一候选的证据等待。冷却只限制重复播放操作，不再拦截已准入候选的后续转写。候选身份和代次一致、仍在原有证据期限内时，后续声学句段复用既有文本组装；已经裁决的候选不会重新打开。

实现复用原有 800 ms 合并保护窗口、6 秒证据预算、候选/代次、文本组装和定时任务入口，没有增加热词、第二套队列或新的 SDK 编排。窗口数值未改，但用途发生明确变化：默认无意图模型路径的播放等待与证据等待分离；语义候选可能跨越声学合并窗口继续存在。已配置意图模型的路径保留等待模型结果的策略。暂时恢复不取消语义稳定性复查；弱证据等待通过既有复查任务到期重评，避免没有后续 ASR 事件就永远等不到裁决。

自动化验证不是只增加原话示例：框架层覆盖中文日期、人数和英文目的地，FINAL 在 VAD 停声前/后到达，前段/后段 EOT 为 0.003/0.424、0.003/0.95、0.4/0.95 的 18 种组合；另覆盖播放恢复后的迟到 FINAL、冷却窗口内外重新开口、长时间仍在说话、无续说时到期回收，以及原模型结果迟到、错误、显式抢占和恢复内容完整性。最终相关单元/基准测试 400 项、框架场景 46 项全部通过。框架场景注入模型边界分数，不冒充真实 ASR/EOT 准确率。

真实 RTC 首轮三条均失败，暴露了仅依靠声学合并窗口和 completed-turn 回调的草案不足；最终移除该回调兜底草案，修复候选生命周期。第二轮功能已形成完整请求，但原报告把同一 RPC 的重复 timeline 快照算成多个请求；现按房间、话轮及请求发送时间去重，另测同话轮两次真实发送仍计为两次。费用用例按 ASR 数字格式验收。失败原始报告保留，没有重写为通过。

冻结运行源码后，原 managed-worker 基准在 Mac 独立 worker/房间使用真实 STT、EOT、Agent RPC、TTS 和 RTC 接收端，最终三条功能用例全部通过。人数片段间分别插入 500/900 ms 静音，费用片段插入 500 ms；这不是测得的 VAD 停顿时长。每条均提交完整前后文、取消旧回复，恰好两次 RPC（初始问题一次、完整修改一次）。未将此结果解释为实体 mobile 外放验收。

| 完整修改请求 | 服务端最后停声 → cancel | 停声 → commit | commit → 首段 TTS 音频 | 停声 → 首段 TTS 音频 |
| --- | ---: | ---: | ---: | ---: |
| 人数，插入 500 ms 静音 | 803 ms | 2501 ms | 1245 ms | 3746 ms |
| 人数，插入 900 ms 静音 | 803 ms | 2501 ms | 1351 ms | 3852 ms |
| 费用，插入 500 ms 静音 | 86 ms | 98 ms | 1028 ms | 1126 ms |

表中按同一个修改请求的时间戳计算，不能将基准报告中跨请求取最大值的阶段相加。三条新用例未配置延迟门槛，因此报告的 SLO 通过标记不代表整体低延迟验收；收到用户停声后的任意音频也可能仍是旧回复。人数请求的低延迟缺口仍明确存在，且表中尚未包含实体扬声器出声延迟；不报告 P95。

冻结版本的 full duplex / half duplex 人类说话画像和 PTT 回归另有 56 项通过、3 项既有严格预期失败。预期失败涉及“好的”附和、否定停止命令和低 EOT 纠正表达，仍作为语义缺口保留，没有计入通过。Ruff 严重错误检查与 `git diff --check` 通过。

本轮未部署到 OPI5 Max 或现有 Mac 业务 worker。LiveKit Agents 1.7.1 的 207 个 Python 文件仍全部与安装 RECORD 匹配；没有修改第三方 SDK。下一步范围应保持为这条完整请求的提交等待和真实设备复验，不扩大成新的语义引擎或输出编排；既有默认意图语义反例仍未宣称解决。

### 确认打断后约 1.7 秒提交等待的归因（2026-09-09）

证据归档：[结束等待因果核查](voice-validation/endpoint-wait-causality.json)。本轮没有修改生产时序、阈值或 SDK；新增六项回归，核实等待是否可以安全消除。

上轮两条真实 RTC 人数用例在完整文本上的 EOT 分数约为 0.424，低于 0.5 的结束门槛。稳定文本足以让 Channel 接受普通打断，但不等于用户话轮已结束。已安装 SDK 在低于门槛时选择原有 3 秒最大结束等待，并以声学停声时刻为起点扣掉已经过去的时间。VAD 停声通知已包含约 500 ms 静音，所以从通知到提交约为 2500 ms；Channel 在通知后约 803 ms 确认打断，剩余约 1698 ms。没有发现取消旧回复后重新启动一段独立 1.7 秒等待的证据。

通过真实 AgentSession / Channel 编排，在模型边界注入相同文本的 EOT 分数，并改变 VAD 报告的静音量，得到以下对照。计时只到提交，不包括 ASR 供应商、LLM、TTS 或扬声器：

| EOT 注入分数 | VAD 已计入静音 | 停声通知 → commit | cancel → commit |
| --- | ---: | ---: | ---: |
| 0.424 | 200 ms | 2802 ms | 1999 ms |
| 0.424 | 500 ms | 2502 ms | 1701 ms |
| 0.95 | 200 ms | 51 ms | 351 ms |
| 0.95 | 500 ms | 1 ms | 300 ms |

高分路径中 cancel 发生在 VAD 停声之前，因此表中 cancel → commit 可以大于停声通知 → commit。结果符合原始声学时钟及快慢两条结束路径，不能把两种起点混作额外开销。

另两条用例先形成已确认的打断，在 VAD 停声后停顿 1.6 秒再补充条件后半句。默认策略均保留前后文并只回复一次；仅在独立测试进程通过公开 `AgentSession.update_options` 将最大结束等待改为 1.5 秒后，两条均在续说前提交了未完成的前半句。该反例否决“确认打断即可结束话轮”和通用压短等待的做法；它不证明 3 秒是全局最优，也不以降低原断言使对照通过。

真实模型的八组人工诊断文本同时出现完整请求低分、未完条件句高分。例如人数修改完整句约 0.424，日期修改完整句约 0.038；“如果费用控制在五百元以内”约 0.889。这不是总体错误率，也不是经过校准的置信度，但足以说明不能围绕人数用例单独调整阈值，或把文本稳定等同于语义结束。下一步应限定为既有 EOT 的输入契约和成对完成/未完表达评估；任何质量改动同时验收完整请求延迟和暂停续说不抢答。

最终完整请求延迟场景文件 23 项通过、3 项既有严格预期失败（未完条件句被现有模型判为完整），包含本轮新增六项。缩短等待的两项失败对照另行保留。Ruff 严重错误及差异检查通过；运行源码与上一提交验证版本哈希一致，SDK 207 个 Python 文件仍与 RECORD 匹配。未重新运行 RTC、未部署或测量实体扬声器；上轮 3.75/3.85 秒的响应缺口仍未解决。本轮收敛结论是查清等待归属、排除有副作用的快捷优化，不把诊断工作描述成延迟已改善。

### 按场景修正验收口径，900 ms 不作为通用等待（2026-09-09）

用户指出慢说、卡顿者可能需要更长等待。本轮采纳的是“按场景和阶段验收”，没有把所有运行超时统一放大，也没有新增用户语速分类器。此前重点讨论的 900 ms 是短附和案例的 `max_speech_start_to_resume_ms`，从用户起声开始计时，包含用户说话时长；它不是全局 EOT 阈值，也不是设备扬声器 SLA。

| 场景 | 应保护或衡量的体验 | 本轮处理 |
| --- | --- | --- |
| 短附和、误触发恢复 | 恢复旧回复及时且内容完整 | 保留原 900 ms 起声预算作为历史短附和基线，不能直接移作慢说预算 |
| 慢说、卡顿、句内停顿 | 后续开口前不抢答，前后句完整且只提交一次 | 用暂停续说场景验收；没有依据把“等待越久”作为通过条件 |
| 较长语音后的恢复 | 区分用户自身说话时长与恢复开销 | 增加可按 case 设置的 `max_speech_stop_to_resume_ms`；这仍是应用层恢复，不是实体出声 |
| 语义完整后的接话 | 尽快产生该请求的新回复 | 三条续说 RTC 用例补入原 2.5 秒目标的服务端必要条件；通过该条件不代表接收端/实体整链路通过 |
| 用户开口后的让声 | 旧 TTS 尽快让出空间 | 不因为用户慢说就统一放宽该阶段；实体播放端仍需独立验收 |
| PTT / half duplex | 以松键或该模式的结束边界验收 | 未修改模式策略，不套用全双工附和的起声预算 |

修正了四处验收问题，复用现有 benchmark 和 timeline，没有新增运行队列、分类层或结束策略：

- `canonical_contains` 的所有片段现在必须出现在同一份文本中，不能跨不同话轮/字段分别命中。新用例进一步要求同一话轮的完整 canonical、commit、RPC、有效回答文本和首段 TTS 时间戳顺序一致；旧音频、未提交文本、其他话轮的回答不能充数。
- 新回复证明目前到应用 TTS 层；接收端归属明确记录为 `not_measured`。现有 `after_user_done` 仍只表示用户输入结束后有音频，不升级解释为该话轮的新回复。
- 没有设置延迟验收项时，SLO 结果为 `not_evaluated`，报告单列未验收数量；功能结果与已设置的延迟门槛分开呈现。
- 恢复事件记录真实应用操作时间和当时的停声边界，不再用稍后的候选裁决结束冒充恢复。若恢复时用户仍在说话，则不产生虚假的“停声后恢复”时长。旧记录若只有无时间戳的临时恢复标记，报告缺少测量，不能拿终局时间代替。

同时移除前轮诊断测试中“低分必须等约 3 秒、cancel 后必须再等超过 1 秒”的断言，保留时长记录、完整文本和高分快速提交检查，由独立暂停续说测试保护不抢答，避免测试把当前慢等待固化成产品要求。长时间仍在说话时恢复旧输出的既有行为仍是体验待评估项，本轮只准确记录，未宣称其合理或已修复。

对上次原始 RTC 记录重新应用新断言：三条同话轮功能均通过；两条人数用例因停声到新 TTS 为 3746/3852 ms 而失败，费用用例 1126 ms 通过服务端 2500 ms 门槛。三条接收端新回复出声均未验收。这是重新分析旧记录，不是新运行或实体复测，旧报告保留。

自动化反例包含跨话轮/字段拼接、未提交文本、旧/缺失音频、仅其他话轮有回复、缺少回答文本和缺少恢复时间戳；还验证相同 200 ms 恢复开销在用户说话 300 ms 与 2 秒时可共享停声后预算，而原起声预算仍保持原含义。证据见 [按场景验收修正](voice-validation/case-aware-acceptance.json)。本轮不依据单次样本重新定产品 SLA，尚未部署或进行 mobile 外放复验。

最终 benchmark/时间线单元测试 208 项、恢复框架场景 7 项、结束边界保护场景 6 项全部通过。恢复框架样本实际记录停声后约 801 ms 恢复输出，而候选在自身开始后约 6702 ms 才结束，两者不能混算。SDK 207 个 Python 文件仍与 RECORD 匹配；Ruff 严重错误及差异检查通过。报告显式列出三条 RTC 样本的接收端新回复均未验收。

### 新回复到达 RTC 接收端的实测归属（2026-09-09）

本轮实际重跑三条续说用例，使用真实 STT、EOT、Agent RPC 和 TTS。现有房间测试入口新增默认关闭的 `--livekit-audio-capture-dir`，保存接收端解码 PCM 和每帧 monotonic 到达时间。隔离 worker 通过临时测试代码旁路记录 Channel 自有 Agent 的 `tts_node` 原始音频及话轮 ID，原包照常向下传递；未改生产编排或第三方 SDK。

用 SciPy 标准归一化互相关，将每条新回复首个有声块开始的两段 400 ms 波形与接收音频匹配，两段相隔 500 ms。三条新回复的两段匹配系数为 0.926–0.987，接收样本间距均与源音频完全一致。把新回复波形放到其产生前的旧音频中搜索，首段最高仅 0.184–0.205；第二段因通用短语可达 0.767，说明不能仅凭任意一段相似音频、音量上涨或同步字幕判定新回复。详细系数、原始文件哈希、话轮映射及临时诊断源码保留在 [RTC 音频归属证据](voice-validation/rtc-reply-audio-attribution.json)。这是离线归属验证，默认报告仍明确标记接收端未自动验收，没有将诊断方法直接升格为通用门禁。

| 用例 | VAD 停声→提交 | 提交→RPC 发出 | RPC→有效回答 | 回答→首 TTS | 首 TTS→接收有声块 | VAD 停声→接收 | 基准末有声帧→接收 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 人数，插入静音 500 ms | 2501 ms | 2 ms | 963 ms | 549 ms | 261 ms | 4275 ms | 4878 ms |
| 人数，插入静音 900 ms | 2501 ms | 2 ms | 569 ms | 447 ms | 269 ms | 3788 ms | 4398 ms |
| 费用，插入静音 500 ms | 27 ms | 1 ms | 609 ms | 675 ms | 284 ms | 1597 ms | 2205 ms |

最后一列复用测试发送端的 `last_voiced_frame` 边界，不是物理麦克风实测；VAD 通知比这个边界晚约 600 ms，必须计入用户说完后的体验。接收时间是解码帧到达消费者的时间，不是 mobile 扬声器出声；首 TTS→接收还包括源音频前导静音、缓冲和调度，不能全归为网络耗时。20 ms 音频块及匹配近似也不支持毫秒级物理精度。

最终三个场景功能通过，现有应用层 2.5 秒门槛仍只有费用场景通过，两个人数场景失败。本轮没有降低门槛或宣称总体延迟达标。人数 500 ms 首次房间在欢迎语完成检测阶段超时，由已有机制重试一次后进入场景；重试原因及原始音频保留，不能视为首试全通过，其根因本轮未确定。

回归覆盖 benchmark、时间线、输出流、交互模式、PTT 分段及 half duplex 打断边界，正确配置已有本地模型后 271 项全部通过。首次运行三项因工作树模型是 LFS 占位而加载失败，未修改模型文件或产品逻辑；记录在 `/tmp/voice-reply-capture-regression.log`，有效模型重跑为 `/tmp/voice-reply-capture-regression-model.xml`。新增录音开启/关闭测试核验原始 PCM、帧偏移、到达时间与资源关闭。Ruff 严重错误及差异检查通过；SDK 1.7.1 的 207 个 Python 文件仍与 RECORD 完全一致。

ADB 已发现原 mobile pad；OPI5 Max 的 Channel 服务在线，发布清单标记版本 `ee4f1da193683a60e92977fd80d135a73816db0b`，与本工作区 `cd070fe` 基线不同。本轮未部署或进行外放验收。主线剩余的是完整请求的 EOT 结束判断质量和说话中让声体验；本次证据支持优先检查既有 EOT 输入及完成/未完表达的成对表现，不能围绕人数用例压短统一等待，更不能把慢说预算放大当成接话延迟优化。接收端通路在这三个样本中不是主要等待来源，但不能从三个样本推断行业水平、尾延迟或所有设备表现。
