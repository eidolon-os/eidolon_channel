...
def _owner() -> SimpleNamespace:
    ...
    owner._cancel_deferred_low_eot_commit = MagicMock()
    owner._cancel_pending_voiceprint_commits = MagicMock()
    ...
    owner._reset_candidate_voiceprint_tasks = MagicMock()
...
def test_speech_lifecycle_stop_defers_when_interruption_owner_waits_for_stt() -> None:
    ...
    owner._remember_candidate_voiceprint_task.assert_called_once_with(voiceprint_task)
...
def test_speech_lifecycle_stop_schedules_voiceprint_gated_commit() -> None:
    ...
    owner._playback_low_evidence_reject_reason = MagicMock(return_value="")
    owner._should_defer_low_eot_commit = MagicMock(return_value=False)
    owner._candidate_voiceprint_gate_task = MagicMock(return_value="verify-task")
    owner._schedule_voiceprint_gated_commit = MagicMock()

    FullDuplexSpeechLifecycle(owner).handle_stopped()

    owner._remember_candidate_voiceprint_task.assert_called_once_with(voiceprint_task)
    owner._user_turns.finish_speech.assert_called_once()
    owner._schedule_voiceprint_gated_commit.assert_called_once_with(
